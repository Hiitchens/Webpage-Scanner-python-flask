...
def routeThisShit():
    ...
    if url is not None and len(url) != 0:
         rawContent = scrape.cleanHTML(scrape.getPageContent(url))

         topWordsList = scrape.getCountDict(scrape.wordList(rawContent))

        #get top ten items from sorted list
         firstTenElements = dict(sorted(topWordsList.items(), key=lambda x: x[1], reverse=True)[:10])
         return render_template('form.html', url=url, firstTenElements=firstTenElements)
    else:
         return render_template('form.html', url=url, firstTenElements=url)
